...
// ../shared/parsing/blockParser.ts
function parseSupplierBlocks(text) {
...
__name(parseSupplierBlocks, "parseSupplierBlocks");
function isSupplierHeader(line) {
...
__name(isSupplierHeader, "isSupplierHeader");
function normalizeHeader(header) {
  return header.replace(/^(SUPPLIER|VENDOR|FROM|ORDER\s+FROM)[:]?\s*/i, "").trim();
}
__name(normalizeHeader, "normalizeHeader");
function createFallbackBlock(text) {
  const lines = text.split(/\n/).map((line) => line.trim()).filter((line) => line.length > 0);
  return {
    supplierName: "",
    lines
...
__name(createFallbackBlock, "createFallbackBlock");

// ../shared/parsing/llmPrompt.ts
function buildExtractionPrompt(blocks) {
  if (blocks.length === 0) {
...
function getBasePrompt(content) {
...
__name(getBasePrompt, "getBasePrompt");
...
__name(buildVisionPrompt, "buildVisionPrompt");

// ../shared/clients/groq.ts
var GROQ_API_URL = "https://api.groq.com/openai/v1/chat/completions";
async function groqVision(payload, apiKey) {
  try {
...
function pdfToBase64DataUrl(buffer, mimeType = "application/pdf") {
...
__name(pdfToBase64DataUrl, "pdfToBase64DataUrl");
...
// ../shared/validation/parsedDoc.ts
var ParsedItemSchema = external_exports.object({
  supplier: external_exports.string().optional(),
  product: external_exports.string().min(1, "Product name is required"),
  quantity: external_exports.number().int().nonnegative().optional()
});
var ParsedDocSchema = external_exports.object({
  items: external_exports.array(ParsedItemSchema)
});
function validateParsedDoc(input) {
  try {
...
__name(validateParsedDoc, "validateParsedDoc");

// ../shared/utils/normalize.ts
function normalize(text) {
  if (!text || typeof text !== "string") {
    return "";
  }
  return text.trim().toLowerCase().replace(/[.!?]{2,}/g, ".").replace(/\s+/g, " ").trim();
}
__name(normalize, "normalize");
function normalizeSupplier(name) {
  const normalized = normalize(name);
  return normalized.replace(/\s+(ltd|limited|inc|incorporated|corp|corporation)\.?$/i, "").trim();
}
__name(normalizeSupplier, "normalizeSupplier");
...
var GROQ_MODEL = "llama-3.1-70b-versatile";
async function handleParseDoc(file, env) {
...
__name(handleParseDoc, "handleParseDoc");
  ...
  async fetch(request, env) {
    try {
      ...
      if (request.method !== "POST") {
        const { response: response2 } = createError("Method not allowed", 405);
        return withCors(response2);
      }
      const contentType = request.headers.get("content-type") || "";
      if (!contentType.includes("multipart/form-data")) {
        const { response: response2 } = createError("Expected multipart/form-data", 400);
        return withCors(response2);
      }
      let formData;
      try {
        ...
        const { response: response2 } = createError("Invalid form data", 400);
        return withCors(response2);
      }
      const file = formData.get("file");
      if (!file) {
        const { response: response2 } = createError("No file uploaded", 400);
        return withCors(response2);
      }
...
// .wrangler/tmp/bundle-iKUQkz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-iKUQkz/middleware-loader.entry.ts
